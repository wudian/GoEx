...
const (
	EXCHANGE_NAME = "hashkey"

	API_BASE_URL = "https://api-preview.pro.hashkey.com/"
	API_V2       = "api/2/"
...
func New(client *http.Client, accessKey, secretKey string) *Hashkey {
	return &Hashkey{accessKey, secretKey, client}
}

func (hashkey *Hashkey) GetExchangeName() string {
	return EXCHANGE_NAME
}

// https://api.hashkey.com/#symbols
/*
curl "https://api.hashkey.com/api/2/public/symbol"
[
  {
    "id": "ETHBTC",
    "baseCurrency": "ETH",
...
*/
func (hashkey *Hashkey) GetSymbols() ([]goex.CurrencyPair, error) {
	resp := []map[string]interface{}{}
	err := hashkey.doRequest("GET", SYMBOLS_URI, &resp)
	if err != nil {
		return nil, err
	}
...
// https://api.hashkey.com/#tickers

/*
curl "https://api.hashkey.com/api/2/public/ticker"
...
*/
func (hashkey *Hashkey) GetTicker(currency goex.CurrencyPair) (*goex.Ticker, error) {
	curr := hashkey.adaptCurrencyPair(currency).ToSymbol("")
	tickerUri := API_BASE_URL + API_V2 + TICKER_URI + curr
	bodyDataMap, err := goex.HttpGet(hashkey.httpClient, tickerUri)
	if err != nil {
		return nil, err
	}
	...
	value := tickerMap["timestamp"].(string)
	timestamp, _ := time.Parse("2006-01-02T15:04:05.000Z", value)
	ticker.Date = timestamp.Unix()
...
curl -X PUT -u "ff20f250a7b3a414781d1abe11cd8cee:fb453577d11294359058a9ae13c94713" \
    "https://api.hashkey.com/api/2/order/d8574207d9e3b16a4a5511753eeef175" \
...
*/
func (hashkey *Hashkey) placeOrder(ty goex.TradeSide, amount, price string, currency goex.CurrencyPair) (*goex.Order, error) {
	postData := url.Values{}
	...
	headers["Authorization"] = "Basic " + base64.StdEncoding.EncodeToString([]byte(hashkey.accessKey+":"+hashkey.secretKey))

	bytes, err := goex.HttpPostForm3(hashkey.httpClient, reqUrl, postData.Encode(), headers)
	if err != nil {
		return nil, err
	}
	...
	*/
	...
	return hashkey.toOrder(resp), nil
}

func (hashkey *Hashkey) LimitBuy(amount, price string, currency goex.CurrencyPair) (*goex.Order, error) {
	return hashkey.placeOrder(goex.BUY, amount, price, currency)
}

func (hashkey *Hashkey) LimitSell(amount, price string, currency goex.CurrencyPair) (*goex.Order, error) {
	return hashkey.placeOrder(goex.SELL, amount, price, currency)
}

func (hashkey *Hashkey) MarketBuy(amount, price string, currency goex.CurrencyPair) (*goex.Order, error) {
	return hashkey.placeOrder(goex.BUY_MARKET, amount, price, currency)
}

func (hashkey *Hashkey) MarketSell(amount, price string, currency goex.CurrencyPair) (*goex.Order, error) {
	return hashkey.placeOrder(goex.SELL_MARKET, amount, price, currency)
}

func (hashkey *Hashkey) CancelOrder(orderId string, currency goex.CurrencyPair) (bool, error) {
	postData := url.Values{}
	reqUrl := API_BASE_URL + API_V2 + ORDER_URI + "/" + orderId
	headers := make(map[string]string)
	headers["Authorization"] = "Basic " + base64.StdEncoding.EncodeToString([]byte(hashkey.accessKey+":"+hashkey.secretKey))
	bytes, err := goex.HttpDeleteForm(hashkey.httpClient, reqUrl, postData, headers)
	if err != nil {
		return false, err
	}
...
func (hashkey *Hashkey) GetOneOrder(orderId string, currency goex.CurrencyPair) (*goex.Order, error) {
	resp := make(map[string]interface{})
	err := hashkey.doRequest("GET", ORDER_URI+"/"+orderId, &resp)
	if err != nil {
		return nil, err
	}
...
func (hashkey *Hashkey) GetUnfinishOrders(currency goex.CurrencyPair) ([]goex.Order, error) {
	params := url.Values{}
	params.Set("symbol", currency.ToSymbol(""))
	resp := []map[string]interface{}{}
	err := hashkey.doRequest("GET", ORDER_URI+"?"+params.Encode(), &resp)
	if err != nil {
		return nil, err
	}
...
// TODO
// https://api.hashkey.com/#orders-history
func (hashkey *Hashkey) GetOrderHistorys(currency goex.CurrencyPair, currentPage, pageSize int) ([]goex.Order, error) {
	params := url.Values{}
	params.Set("symbol", currency.ToSymbol(""))
	resp := []map[string]interface{}{}
	err := hashkey.doRequest("GET", ORDER_URI+"?"+params.Encode(), &resp)
...
// https://api.hashkey.com/#account-balance
func (hashkey *Hashkey) GetAccount() (*goex.Account, error) {
	var ret []interface{}
	err := hashkey.doRequest("GET", BALANCE_URI, &ret)
	if err != nil {
		return nil, err
	}
...
// https://api.hashkey.com/#orderbook
/*
{
  "ask": [
    {
...
*/
func (hashkey *Hashkey) GetDepth(size int, currency goex.CurrencyPair) (*goex.Depth, error) {
	params := url.Values{}
	params.Set("limit", fmt.Sprintf("%v", size))
	resp := map[string]interface{}{}
	curr := hashkey.adaptCurrencyPair(currency).ToSymbol("")
	err := hashkey.doRequest("GET", DEPTH_URI+"/"+curr+"?"+params.Encode(), &resp)
	if err != nil {
		return nil, err
	}
...
// https://api.hashkey.com/#candles

/*
curl "https://api.hashkey.com/api/2/public/candles/ETHBTC?period=M30"
[
  {
...
*/
func (hashkey *Hashkey) GetKline(currencyPair goex.CurrencyPair, period string, size, since int64) ([]goex.Kline, error) {
	switch period {
	...
	resp := []map[string]interface{}{}
	err := hashkey.doRequest("GET", KLINE_URI+"/"+currencyPair.ToSymbol("")+"?"+params.Encode(), &resp)
	if err != nil {
		return nil, err
	}
...
// https://api.hashkey.com/#trades

/*
curl "https://api.hashkey.com/api/2/public/trades/ETHBTC?from=2018-05-22T07:22:00&limit=2"
[
   {
      "id" : 297604734,
...
*/
func (hashkey *Hashkey) GetTrades(currencyPair goex.CurrencyPair, since int64) ([]goex.Trade, error) {
	params := url.Values{}
	timestamp := time.Unix(since, 0).Format("2006-01-02T15:04:05")
	params.Set("from", timestamp)
	resp := []map[string]interface{}{}
	err := hashkey.doRequest("GET", TRADES_URI+"/"+currencyPair.ToSymbol("")+"?"+params.Encode(), &resp)
	if err != nil {
		return nil, err
	}
...
func (hashkey *Hashkey) doRequest(reqMethod, uri string, ret interface{}) error {
	url := API_BASE_URL + API_V2 + uri
	req, _ := http.NewRequest(reqMethod, url, strings.NewReader(""))
	req.SetBasicAuth(hashkey.accessKey, hashkey.secretKey)
	resp, err := hashkey.httpClient.Do(req)
	if err != nil {
		return err
	}
...
// https://api.hashkey.com/#order-model
{
	"id": 0,
	"clientOrderId": "d8574207d9e3b16a4a5511753eeef175",
	"symbol": "ETHBTC",
...
*/
func (hashkey *Hashkey) toOrder(resp map[string]interface{}) *goex.Order {
	return &goex.Order{
		...
		Currency:   hashkey.adaptSymbolToCurrencyPair(resp["symbol"].(string)),
...
func (hashkey *Hashkey) adaptCurrencyPair(pair goex.CurrencyPair) goex.CurrencyPair {
	return pair.AdaptUsdtToUsd().AdaptBccToBch()
}

func (hashkey *Hashkey) adaptSymbolToCurrencyPair(pair string) goex.CurrencyPair {
